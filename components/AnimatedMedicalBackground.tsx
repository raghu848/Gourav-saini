...
const AnimatedMedicalBackground = () => {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      {/* Simplified Animated DNA Helix - Hidden on mobile */}
      <motion.div
        key="dna-helix"
        className="absolute right-10 top-20 opacity-20 hidden lg:block"
        animate={{
          rotate: 360,
          y: [-8, 8, -8]
        }}
      ...
      >
        <svg width="100" height="250" viewBox="0 0 100 250" className="text-blue-300">
          ...
          {[...Array(5)].map((_, i) => (  // Reduced from 10 to 5 for performance
            <g key={`dna-group-${i}`}>
              <motion.path
                key={`dna-path-1-${i}`}
      ...
      {/* Simplified Animated Spine - Hidden on mobile */}
      <motion.div
        key="spine"
        className="absolute left-10 top-1/4 opacity-15 hidden lg:block"
        animate={{
          y: [-5, 5, -5],
        }}
        transition={{
